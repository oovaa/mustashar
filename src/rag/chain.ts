...
export async function answer(question: string, summary?: string) {
  ...
  /* ===============================
    ...
    4.يرجى الإجابة باللغة العربية الفصحى فقط وبشكل صارم. يُمنع استخدام أي حروف أو كلمات من لغات أخرى (مثل الصينية، التايلاندية، أو الإنجليزية)، حتى في المصطلحات القانونية أو أدوات الربط. إذا احتجت لمصطلح تقني، استخدم المرادف العربي القياسي له فقط
    5.أرجع نص البحث فقط (بدون أي مقدمات أو شرح إضافي).
    6. هام جداً: إذا كان إدخال المستخدم مجرد تحية (مثل "السلام عليكم"، "مرحبا")، أو مجرد حرف/علامة ترقيم (مثل ".")، أو لا يحتوي على أي سؤال قانوني واضح، يجب عليك أن ترجع الكلمة التالية حرفياً فقط وبدون أي إضافات: NO_SEARCH
    7. قبل عرض الإجابة، قم بمراجعة كل حرف للتأكد من أنه ينتمي إلى الأبجدية العربية فقط. إذا وجدت أي رموز أو حروف غير عربية، قم بترجمتها إلى العربية فوراً قبل إرسال الرد
    `,
      ],
